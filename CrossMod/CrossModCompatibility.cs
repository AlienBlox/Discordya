...
namespace DiscordyaV2.CrossMod
{
	public class CrossModCompatibility
	{
		...
		{
			this._externalModDictionary = (Dictionary<CrossMods, float>)null;
		...
		private void SetupCrossModCompatibility()
		{
			...
			if (calamity != null)
			{
				this.AddMod(new CrossMods("calamity", "844487120416407573", "calamity"), 3f);
		  ...
		  (object) "bossrush"
			...
			if (mod1 != null)
			{
				this.AddMod(new CrossMods("starsabove", "824723328580976660", "starsabove"), 2f);
				ModNPC modNpc35;
			...
			if (mod2 == null)
				return;
			this.AddMod(new CrossMods("gensokyo", "833828769260371978", "gensokyo"), 1f);
			...
			this._discordyaMod.GetGamePresenceProcessor().AddBoss(new List<int>()
	  {
		...
		{
			return new List<CrossMods>((IEnumerable<CrossMods>)this._externalModDictionary.Keys);
		}

		internal CrossMods GetPreferredMod()
		{
			return this._externalModDictionary.OrderByDescending<KeyValuePair<CrossMods, float>, float>((Func<KeyValuePair<CrossMods, float>, float>)(x => x.Value)).Select<KeyValuePair<CrossMods, float>, CrossMods>((Func<KeyValuePair<CrossMods, float>, CrossMods>)(x => x.Key)).First<CrossMods>();
		}
	}
}
